...
| Platform services | `msplatformservices/{wns,entra,graph}` | WNS push, Entra token validation, Graph | `wns` implemented (record 0018); `entra` and `graph` are placeholders (Phase 10) |
...
| Server | `server/{sqlstore,service,httpapi,pushnotify,agent,windcsync,adminauth,audit,eventsink}` | Persistence, orchestration, transport, administration | `sqlstore`, `service`, `httpapi`, `pushnotify`, `agent/{enroll,wake}` implemented; `windcsync`, `adminauth`, `audit`, `eventsink` are placeholders (Phases 12, 15) |
