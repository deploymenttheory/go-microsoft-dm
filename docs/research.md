...
`go-microsoft-dm` will be a pure Go library and reference server for managing Windows 11 devices with the
...
- Notes: why it matters for go-microsoft-dm, what is newer or older than it, anything it contradicts
```
...
Getting 26H2 media before general availability: go-sdk-winmediafoundry's Windows Update client
(section 5) queries the ReleasePreview ring for amd64 or arm64 and builds bootable media from the
ESD, so a guestweave guest can run 26300.x now; the `latest` catalog release stays 25H2 until
Microsoft's Media Creation Tool catalog moves.
...
- Notes: The set MS-MDM normatively cites and the Learn OMA-DM page points to for Security. Treat it as the primary OMA reference for go-microsoft-dm.
...
- Notes: Best end-to-end reference for the go-microsoft-dm enrollment server; uses RequestVersion 3.0 samples (MDE2 allows up to 9.0). The sample RSTR envelope uses the SOAP 1.1 namespace while requests use SOAP 1.2; follow MDE2 2.1 with care and test against a real client.
...
- Notes: Only needed if go-microsoft-dm wants to drive the ESP experience; the DMClient FirstSyncStatus nodes are the server-facing half.
...
- Notes: Do not cite as a separate source; go-microsoft-dm should treat `Policy.xml` plus `*_AreaDDF.xml` in the bundle as the Policy DDF.
...
- Notes: Essential for go-microsoft-dm's Policy CSP encoder. DDF `AllowedValues ValueType="ADMX"` plus `<MSFT:AdmxBacked Area= Name= File=>` gives the ADMX file and policy per node (2,518 such nodes in the Feb 2026 bundle) so the generated schema can carry the link.
...
- Notes: The wire spec for the go-microsoft-dm push sender. Combine with the MDM page's "set X-WNS-Cache-Policy: cache" advice, which overrides the raw default of no-cache.
...
- Notes: Anecdotal, with an internal contradiction (the question says no session; the final update says the session does run despite 4603). No Microsoft acknowledgement, KB, or workaround; the advisor's payload theory was falsified in-thread. Takeaway for go-microsoft-dm: send a non-empty raw body, keep `X-WNS-Cache-Policy: cache`, never rely on push alone, re-read ChannelURI each session, and log Event 4603 as a known ambiguous symptom on 24H2 and 25H2 until Microsoft documents otherwise.
...
- Notes: These URL fields are exactly what the Entra admin pastes for a third-party MDM. Programmable via `mobileDeviceManagementPolicies` (beta), which a go-microsoft-dm setup command could automate.
...
- Notes: The practical path for go-microsoft-dm: populate `FirstSyncStatus/ExpectedPolicies`, `ExpectedMSIAppPackages`, `ExpectedModernAppPackages`, `ExpectedSCEPCerts`, and set `IsSyncDone` and `WasDeviceSuccessfullyProvisioned` to feed the built-in OOBE progress. Full ESP app-blocking with the Intune Management Extension sidecar is an Intune experience.
...
- Notes: Directly usable: go-microsoft-dm can be a bulk-enrollment target with AuthPolicy OnPremise or Certificate in the ppkg, and Malcolm/local-mdm already generates and signs such packages (section 3). "Bulk-join isn't supported in Microsoft Entra join" for third-party targets.
...
- Notes: Out of scope for go-microsoft-dm. Recorded only to mark that Cloud PC management is Intune-bound.
...
- Notes: The model for go-microsoft-dm's own simulator package, in the same role as `jessepeterson/mdmb` for Apple.
...
- Notes: The first thing to read when an enrollment against go-microsoft-dm fails.

#### Test environments: guestweave (same organisation)
- Repo: <https://github.com/deploymenttheory/guestweave-cli-windows> · Go · MIT · Active (last push 2026-09-01; pre-alpha) · 0 stars; <https://github.com/deploymenttheory/guestweave-cli-macos> · Go · MIT · Active (last push 2026-09-03) · 0 stars; <https://github.com/deploymenttheory/guestweave-agent> · Go · MIT · Active (last push 2026-09-01); <https://github.com/deploymenttheory/go-sdk-vtpm2> · Go · MIT · Active, GA v1.0.2 (last push 2026-08-26) · public; <https://github.com/deploymenttheory/go-sdk-winmediafoundry> · Go · MIT · Active, preview v0.8.0 (last push 2026-09-05) · public
- Note: the three guestweave repositories are private on 2026-09-07; they resolve only with an authenticated GitHub session, so the link check reports them as 404. go-sdk-vtpm2 and go-sdk-winmediafoundry are public.
...
- Notes: The project's test environments come from these two CLIs; no third-party hypervisor tooling is needed. Windows media on both hosts comes from go-sdk-winmediafoundry, a pure Go toolkit (no wimlib, DISM, oscdimg or cabextract) with three acquisition clients and an imaging library: a Windows Update client that discovers builds by ring (Canary, Beta, ReleasePreview, Retail, and others) and architecture (amd64, arm64, x86) and streams the ESD and CAB files from Microsoft's CDN; a Media Creation Tool catalog client for release ESDs (21H2 to 25H2 by marketing name, plus `latest`); a consumer software-download client that reproduces the Windows 11 ISO download flow; and WIM/ESD readers, a UDF writer and an El Torito ISO builder that turn an ESD into bootable media in one call. On a Windows host, `weave create win --from-windows pro-25h2` (editions pro, home, enterprise, education; release 21h2 to 25h2 or `latest`) resolves the edition through the catalog, downloads and caches the ESD with hash verification, builds the ISO, attaches an answer file on a side volume, and installs unattended into a local admin (`weave`/`weave`) with OOBE skipped, OpenSSH enabled and a completion marker; `--from-windows <iso>` takes a prepared ISO by path. On macOS, `weave media build` uses the same toolkit to acquire pristine ARM64 media and re-master instrumented profiles (`debug`, `kd`, `boot`). Because the Windows Update client can select the ReleasePreview ring, 26H2 (26300.x) media is reachable today, before general availability, which is what the section 0 target needs. The guest runs on the Host Compute Service in guest-isolation mode and gets a real TPM 2.0 and Secure Boot by default (`Get-Tpm` reports MSFT, `Confirm-SecureBootUEFI` True, BitLocker turns itself on), so the Windows 11 install checks are satisfied rather than bypassed; `--no-vtpm` gives a plain VM. The vTPM state lives in the guest-state file and travels with snapshots and clones, so a snapshot of a clean, un-enrolled guest can be reverted to reset an enrollment without breaking BitLocker. `weave serve` exposes create, start, stop, snapshot revert, IP resolution, an MJPEG console and agent calls over HTTP (`/weave/vms/...`, bearer token, OpenAPI at `/openapi.json`) or MCP, and `weave agent call <vm> <tool>` runs tools inside the guest; that is the seam an e2e harness uses to enroll a real Windows client against go-microsoft-dm, capture SyncML with SyncMLViewer, and revert. On a macOS host, `weave create --from-windows 11 <name>` creates a Windows 11 ARM64 guest on guestweave's own Hypervisor.framework VMM (the Virtualization framework cannot boot Windows on ARM), with a TPM 2.0 provided in-process by go-sdk-vtpm2, a pure Go TPM built from the TCG specification (125 of 136 commands, EK and SRK provisioning, PCRs, sealing, Quote, Certify, MakeCredential and ActivateCredential, versioned persistent state), exposed to the guest over the standard CRB interface with a `MSFT0101` ACPI identity so Windows drives it with its stock driver; the guest agent handles Windows shutdown and restart, and `winmedia` builds the ARM64 install media. Either host therefore gives a Windows 11 guest with a TPM 2.0. Two limits carry over from Microsoft, not from guestweave: Intune's enrollment attestation feature is unsupported on any VM, but go-microsoft-dm is itself the verifier of the MS-MDE2 AIK claims it receives, so that flow can be exercised against a vTPM guest with trust anchored to the emulator's provisioned endorsement key, and only validation against vendor EK certificate chains needs physical hardware; and the unattended profile skips OOBE, so Autopilot and OOBE-time enrollment flows need the retail or noprompt media profile instead.
...
same wire protocol go-microsoft-dm must speak, but it is marked Intune-gated.
...
- <https://oliverkieselbach.com/2022/09/21/deep-dive-of-scep-certificate-request-renewal-on-intune-managed-windows-clients/> · blog · 2022-09-21 · Maintenance · partial. `omadmclient.exe`, ClientCertificateInstall CSP, the encrypted one-hour SCEP challenge, `dmcertinst.exe` scheduled task, NodeCache-driven renewal by thumbprint. The client-side flow go-microsoft-dm must emit profiles for.
...
- Notes: The closest public prior art to go-microsoft-dm as a design narrative; the code is in section 3. The enrolling user must be a local admin.
...
| Pitfall | Source and date | What go-microsoft-dm does about it |
|---|---|---|
...
| `0x80180032` (IsTPMAttested) blocks enrollment when an Intune enrollment restriction requires attestation. | call4cloud 2024-07-31 | Not something a third-party MDM sets, but go-microsoft-dm can read attestation via the MDE2 AdditionalContext items and choose to reject non-attested enrollments itself. |
...
## 9. Observations for go-microsoft-dm
...
- No open source project implements WinDC server-side. The protocol is fully documented on Learn
  ...
  go-microsoft-dm, in the same way DDM was for go-apple-dm, and it is the part with the least prior art.
...
- Only local-mdm ships a working WNS client. Fleet built one, proved it wakes devices, and turned it
  off in favour of its agent. go-microsoft-dm has no agent, so WNS push plus a sane poll schedule is the
...
- Test environments are solved in-house. guestweave on either host gives a Windows 11 guest with a
  ...
  go-microsoft-dm's e2e tier can enroll a real client, capture the SyncML, and revert to a clean snapshot
  ...
  go-microsoft-dm is the verifier; only vendor EK certificate chain validation needs physical hardware.
...
7. How go-microsoft-dm relates to `deploymenttheory/go-sdk-windowscsp`. Decided 2026-09-06: it is proof
...
9. Whether Microsoft will publish anything new for MDM in 26H2 between Release Preview and GA. Every
   page checked was silent. A Release Preview guest built through winmediafoundry (section 5) can
   answer the empirical half now: diff `DeviceManageability/Capabilities/CSPVersions` and a DDF
   dump between a 26100 and a 26300 guest.

Questions 1, 2, 4, 5 and 6 are closed by a guestweave Windows 11 guest (section 5) running
SyncMLViewer against a go-microsoft-dm test server; none of them needs physical hardware, including the
attestation items behind question 1, which a vTPM guest produces and go-microsoft-dm verifies.
